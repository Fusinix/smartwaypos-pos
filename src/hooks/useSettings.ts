...
export const useSettings = () => {
	...
	const addUser = async (user: NewUser) => {
		try {
			...
			const users = await window.electron.invoke("add-user", {
				...user,
				author: adminUser,
			});
	...
	const updateUser = async (id: number, user: Omit<User, "id">) => {
		try {
			...
			await window.electron.invoke("update-user", id, {
				...user,
				author: adminUser,
			});
	...
	const exportDatabase = async () => {
		try {
			...
			const data = await window.electron.invoke("export-database", "all", {
				author: adminUser,
			});
	...
	const importDatabase = async (data: string) => {
		try {
			...
			await window.electron.invoke("import-database", data, "all", {
				author: adminUser,
			});
			const updatedSettings = await window.electron.invoke("get-settings");
			const updatedUsers = await window.electron.invoke("get-users");
	...
	const clearAllData = async () => {
		try {
			...
			setUsers(updatedUsers);
			showSuccess(
				"All data cleared successfully. Default admin user has been recreated.",
			);
		} catch (err) {
