...
export const useFood = () => {
	...
	const addFoodItem = useCallback(
		async (item: NewFoodItem) => {
			try {
				...
				const newItem = await window.electron.invoke("add-food-item", item, {
					author,
				});
	...
	const updateFoodItem = useCallback(
		async (id: number, item: Partial<NewFoodItem>) => {
			try {
				setError(null);
				const author = {
					id: user?.id,
				...
				const updatedItem = await window.electron.invoke(
					"update-food-item",
					{ id, ...item },
					{ author },
				);
	...
	const addFoodCategory = useCallback(
		async (category: NewFoodCategory) => {
			try {
				setError(null);
				const author = {
					id: user?.id,
				...
				const newCategory = await window.electron.invoke(
					"add-food-category",
					category,
					{ author },
				);
	...
	const updateFoodCategory = useCallback(
		async (id: number | string, category: Partial<NewFoodCategory>) => {
			try {
				setError(null);
				const author = {
					id: user?.id,
				...
				const updatedCategory = await window.electron.invoke(
