...
import { EditFoodItemDialog } from "@/components/orders/EditFoodItemDialog";
...
export const EditOrder: React.FC = () => {
	...
	// Redirect if no order is being edited
	useEffect(() => {
		if (!editingOrder) {
			navigate("/orders");
		}
	}, [editingOrder, navigate]);
	...
	useEffect(() => {
		if (editingOrder?.items) {
			const items = editingOrder.items as OrderItemDetail[];
