...
const ReportCard = ({ title, description }: any) => (
	<div className="bg-white border border-gray-100 p-6 rounded-xl group hover:border-primary/20 transition-all shadow-sm">
		<div className="flex items-start justify-between mb-4">
			...
			<div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
				<button
					className="p-2 bg-gray-50 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-900 transition-colors"
					title="Download CSV"
				>
		...
		<div className="flex gap-3">
			<Button
				variant="outline"
				className="flex-1 bg-white border-gray-200 text-gray-700 hover:bg-gray-50 h-10 text-xs font-bold rounded-lg"
			>
				CSV
			</Button>
	...
	const reports = [
		{
			title: "P&L Statement",
			description:
				"Comprehensive view of revenue, COGS, and all expenses for the selected period.",
			type: "dashboard",
		},
		{
			title: "Expense Report",
			description:
				"Detailed breakdown of all spending categorized by supplier and type.",
			type: "expenses",
		},
		{
			title: "Payroll Summary",
			description:
				"Staff hours, rates, and total labor costs across the organization.",
			type: "payroll",
		},
		{
			title: "Tax & VAT Ledger",
			description:
				"Detailed tax calculations, collections, and net payable summaries.",
			type: "tax",
		},
		{
			title: "Cash Flow Analysis",
			description:
				"Expected vs. actual cash counts, mobile money splits, and variances.",
			type: "cash",
		},
		{
			title: "Full Accounting Package",
			description:
				"Consolidated report including all of the above for external accountants.",
			type: "dashboard",
		},
	];
		...
		<div className="space-y-8 animate-in fade-in duration-500">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				...
				<div className="bg-white border border-gray-100 p-6 rounded-xl shadow-sm">
					...
					<p className="text-gray-500 text-xs font-medium">
						Auto-formatted for accountants
					</p>
				</div>
			</div>
			...
			<div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{reports.map((report, idx) => (
						<div
							key={idx}
							className="bg-white border border-gray-100 p-6 rounded-xl group hover:border-primary/20 transition-all shadow-sm"
						>
							<div className="flex items-start justify-between mb-4">
								<div className="p-3 bg-gray-50 rounded-xl group-hover:bg-primary/10 transition-colors">
									<FileText className="size-6 text-gray-500 group-hover:text-primary transition-colors" />
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
					<div className="space-y-2">
						...
						<div className="relative">
							...
							<Input
								...
								className="bg-gray-50 border-gray-200 text-gray-900 pl-10 h-12 rounded-xl focus:ring-primary/20"
							/>
						</div>
					</div>
					<div className="space-y-2">
					...
					<div className="flex items-end">
						<Button
							...
							disabled={isExporting || !fromDate || !toDate || !reportType}
