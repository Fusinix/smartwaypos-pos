...
import { Users, Clock, Briefcase, TrendingDown } from "lucide-react";
...
const PayrollTab: React.FC<any> = ({
	...
	const totalLaborCost = payrollExpenses.reduce(
	...
	const totalHours = shifts.reduce(
		(sum: number, s: any) => sum + (s.total_hours || 0),
		0,
	);
	...
	const staff = users.map((u: any) => {
		...
		const userTotalHours = userShifts.reduce(
			(sum: number, s: any) => sum + (s.total_hours || 0),
			0,
		);
		...
		<div className="space-y-8 animate-in fade-in duration-500">
			<div className="flex items-center justify-between">
				<h2 className="text-xs font-bold text-gray-500 uppercase tracking-widest">
					Payroll — {filters?.timePeriod || "Current Period"}
				</h2>
			</div>
			...
			<div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
				<div className="p-6 border-b border-gray-100 flex items-center justify-between">
					...
					<Badge
						variant="outline"
						className="bg-gray-50 border-gray-200 text-gray-700 font-normal px-3 py-1"
					>
						April 2026
					</Badge>
				</div>
							...
							{staff.map((member: any) => (
								...
								>
									<td className="px-6 py-4">
										<div className="flex items-center gap-3">
											<Avatar className="size-8 border border-gray-100">
												<AvatarFallback
													className={cn("text-[10px] font-bold", member.color)}
												>
													{member.initials}
												</AvatarFallback>
											</Avatar>
									...
									<td className="px-6 py-4 text-sm text-gray-500 text-right">
										GH₵ {member.rate.toFixed(1)}
									</td>
									<td className="px-6 py-4 text-sm font-bold text-gray-900 text-right">
										{formatCurrency(member.pay)}
									</td>
									<td className="px-6 py-4 text-sm text-gray-500 text-right">
										{totalLaborCost > 0 ?
											((member.pay / totalLaborCost) * 100).toFixed(1)
										:	0}
										%
									</td>
								</tr>
			...
			<div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm">
				<div className="flex items-center justify-between mb-8">
					...
					<Badge className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 border-0 font-bold">
						Target &lt;35%
					</Badge>
				</div>
