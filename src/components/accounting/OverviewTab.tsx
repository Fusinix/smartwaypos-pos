	...
	<div className="bg-white border border-gray-100 p-6 rounded-xl shadow-sm">
		...
		{delta && (
			...
			>
				...
				{typeof delta === "number" ? `${delta.toFixed(2)}%` : delta} vs last
				month
			</div>
		)}
		{secondaryLabel && (
...
const OverviewTab: React.FC<any> = ({
	...
	return (
		<div className="space-y-8 animate-in fade-in duration-500">
			...
			<div>
				<h2 className="text-xs font-bold text-gray-500 mb-4">
					P&L & Cost Analysis
				</h2>
							...
							<div className="pt-4 border-t border-gray-100">
								<div className="flex justify-between text-lg font-bold">
									...
									<span className="text-emerald-500">
										{formatCurrency((stats?.revenue || 0) - totalExpenses)}
									</span>
								</div>
							</div>
						</div>
					...
					<div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm">
						<div className="flex items-center justify-between mb-8">
							...
							<span className="bg-primary/10 text-primary text-xs px-3 py-1 rounded-full font-bold">
								Prime cost{" "}
								{stats?.revenue > 0 ?
									((totalExpenses / stats.revenue) * 100).toFixed(1)
								:	0}
								%
							</span>
						</div>
						...
						<div className="space-y-8">
								...
								{[
									...
									},
									{
										...
										value:
											stats?.revenue > 0 ?
												(
													((stats.revenue - totalExpenses) / stats.revenue) *
													100
												).toFixed(1)
											:	0,
							...
							<div className="pt-6 border-t border-gray-100">
								...
								<p className="text-gray-500 text-xs text-balance">
									Ratios are based on current period expenses and revenue.
								</p>
							</div>
						</div>
					</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					...
					<div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
						<div className="flex items-center justify-between mb-6">
							...
							<span className="bg-primary/10 text-primary text-[10px] px-2 py-0.5 rounded-full font-bold tracking-wider">
								Live
							</span>
						</div>
						<div className="space-y-4">
					...
					<div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
						<div className="flex items-center justify-between mb-6">
							...
							<span className="bg-blue-500/10 text-blue-500 text-[10px] px-2 py-0.5 rounded-full font-bold tracking-wider">
								Estimated
							</span>
						</div>
						<div className="space-y-4">
							...
							<div className="flex justify-between text-xs py-2 border-b border-gray-50">
								...
								<span className="text-gray-900 font-medium">
									{formatCurrency((stats?.revenue || 0) * 0.12)}
								</span>
							</div>
							<div className="flex justify-between text-xs py-2 border-b border-gray-50">
								<div className="flex flex-col">
								...
								<span className="text-gray-900 font-medium">
									{formatCurrency((stats?.revenue || 0) * 0.025)}
								</span>
							</div>
							<div className="flex justify-between text-xs py-2">
								<span className="text-gray-900 font-bold">Total due</span>
								<span className="text-primary font-bold">
									{formatCurrency((stats?.revenue || 0) * 0.145)}
								</span>
							</div>
						</div>
					</div>
			...
			{/* Export Bar */}
			<div className="bg-white border border-gray-200 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
