	...
	<div className="bg-white border border-gray-100 p-6 rounded-xl shadow-sm">
		...
		<h3
			className={cn(
				...
				isNegative ? "text-rose-500" : "text-gray-900",
			)}
		>
			{value}
		</h3>
...
const CashReconciliationTab: React.FC<any> = ({
	stats,
	analytics,
	filters,
}) => {
	const { paymentMethods = [] } = analytics;
	const cashMethod = paymentMethods.find((p: any) =>
		p.method?.toLowerCase().includes("cash"),
	);
	const otherMethods = paymentMethods.filter(
		(p: any) => !p.method?.toLowerCase().includes("cash"),
	);
	...
	const formatCurrency = (val: number) =>
		`GH₵ ${val?.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;

	const paymentData = {
		labels:
			paymentMethods.length > 0 ?
				paymentMethods.map(
					(p: any) => `${p.method} ${p.percentage.toFixed(1)}%`,
				)
			:	["No Data"],
		datasets: [
			{
				data:
					paymentMethods.length > 0 ?
						paymentMethods.map((p: any) => p.revenue)
					:	[1],
				backgroundColor: [
					"#10b981",
					"#3b82f6",
		...
		<div className="space-y-8 animate-in fade-in duration-500">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				<StatCard
					...
					subValue={`${cashMethod?.count || 0} transactions`}
					icon={Wallet}
				/>
				<StatCard
					title="Card / Mobile Money"
					value={formatCurrency(otherTotal)}
					subValue={`${otherMethods.reduce((sum: number, p: any) => sum + p.count, 0)} transactions`}
					icon={Smartphone}
				/>
				<StatCard
					title="Total variance"
					value="GH₵ 0"
					subValue="No discrepancy reported"
					icon={CheckCircle2}
					isNegative={false}
				/>
			</div>
			...
			<div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
				<div className="p-6 border-b border-gray-100 flex items-center justify-between">
					...
					<Badge
						variant="outline"
						className="bg-gray-50 border-gray-200 text-gray-700 font-normal px-3 py-1 capitalize"
					>
						{filters?.timePeriod || "Current Period"}
					</Badge>
				</div>
						...
						<tbody className="divide-y divide-gray-100">
							{paymentMethods.length === 0 ?
								<tr>
									<td
										colSpan={5}
										className="px-6 py-12 text-center text-gray-500 text-sm italic"
									>
							...
							:	<tr className="hover:bg-gray-50 transition-colors group">
									...
									<td className="px-6 py-4 text-sm text-gray-900 font-bold text-right">
										{formatCurrency(cashTotal + otherTotal)}
									</td>
									<td className="px-6 py-4 text-sm text-gray-900 font-bold text-right">
										{formatCurrency(cashTotal + otherTotal)}
									</td>
						...
						{paymentMethods.map((item: any, idx: number) => (
							<div key={item.method} className="text-center">
								<div className="flex items-center justify-center gap-2 mb-1">
									...
									/>
									<span className="text-[10px] text-gray-500 uppercase font-bold capitalize">
										{item.method}
									</span>
								</div>
				...
				<div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm">
					...
					<div className="mt-8 p-4 bg-gray-50 rounded-xl">
						<p className="text-xs text-gray-500 leading-relaxed">
							All variances are logged automatically at end-of-day drawer count.
							Alerts above GH₵ 50 notify the manager.
						</p>
					</div>
				</div>
			</div>
