...
import { Percent, FileText, Calendar, ChevronDown } from "lucide-react";
...
const TaxTab: React.FC<any> = ({ stats, analytics, filters }) => {
	...
	const taxChartData = {
		labels:
			analytics?.salesData?.map((s: any) => s.date.split("-").pop()) || [],
		datasets: [
			{
				label: "Tax Due",
		...
		<div className="space-y-8 animate-in fade-in duration-500">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				<StatCard
					...
					subValue={filters?.timePeriod || "Current period"}
					icon={FileText}
				/>
				<StatCard
					title="Total tax due"
					value={formatCurrency((stats?.revenue || 0) * 0.18)}
					subValue="Estimated (18%)"
					icon={Percent}
				/>
				<StatCard
					title="Filing status"
					value="Not filed"
					subValue="Requires manual filing"
					icon={Calendar}
				/>
			</div>
				...
				<div className="bg-white border border-gray-100 rounded-xl p-8 shadow-sm">
					...
					<div className="space-y-4">
						{[
							...
							{
								label: "NHIL (2.5%)",
								value: formatCurrency((stats?.revenue || 0) * 0.025),
							},
							{
								label: "GETFL (2.5%)",
								value: formatCurrency((stats?.revenue || 0) * 0.025),
							...
							{
								label: "Tourism levy (1%)",
								value: formatCurrency((stats?.revenue || 0) * 0.01),
							},
						].map((item, idx) => (
							<div
								key={idx}
							...
							>
								<span
									className={cn(
										"text-sm",
										item.isMain ? "text-gray-900 font-bold" : "text-gray-500",
									)}
								>
									{item.label}
								</span>
						...
						<div className="pt-6 border-t border-gray-200">
							<div className="flex justify-between items-center">
								...
								<span className="text-2xl font-bold text-primary">
									{formatCurrency((stats?.revenue || 0) * 0.18)}
								</span>
							</div>
						</div>
					</div>
