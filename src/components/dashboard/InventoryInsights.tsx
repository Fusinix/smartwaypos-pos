...
export const InventoryInsights: React.FC<InventoryInsightsProps> = ({
		...
		if (active && payload && payload.length) {
			...
			return (
				<div className="bg-white p-3 border rounded-lg shadow-lg">
					...
					<p className="text-blue-600">Stock: {data.stock}</p>
	...
	const avgTurnoverRate =
		data.length > 0 ?
			data.reduce((sum, item) => sum + item.turnoverRate, 0) / data.length
		:	0;
	...
	return (
		<Card>
			...
			<CardContent>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
					<div className="bg-muted/60 p-4 rounded-lg">
						...
						<div className="text-2xl font-bold line-clamp-1">
							<span className="font-black text-sm">GHS </span>
				...
				{data?.find((item: any) => item?.turnoverRate > 0) ?
					<div className="mb-6">
