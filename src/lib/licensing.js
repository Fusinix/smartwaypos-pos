...
class LicensingManager {
    ...
    async validateOnline(licenseKey) {
        try {
            ...
            if (result.valid) {
                ...
                await this.saveLicense(result.licenseInfo);
                return result;
            }
            return { valid: false, message: result.message || 'Invalid license' };
        }
        catch (error) {
            console.log('Online validation failed, falling back to offline');
            throw error;
        }
    }
    validateOffline(licenseKey) {
        try {
            ...
            // Verify hardware ID
            if (savedLicense.hardwareId !== this.hardwareId) {
            ...
            if (this.isOfflineGracePeriodExpired(savedLicense)) {
                return {
                    valid: false,
