        ...
        if (Array.isArray(items)) {
            for (const item of items) {
                if (item.itemType === "drink" && item.productId) {
                    ...
                    // Log stock update
                    const product = await db.get("SELECT name, stock FROM products WHERE id = ?", [item.productId]);
                    if (product) {
                        await logAction({
                            db,
                            admin_id: author.id || null,
                    ...
                    if (item.extraIds &&
                        ...
                        console.log(`Inserting ${item.extraIds.length} extras for order item ${orderItemId}`);
                        for (const extraId of item.extraIds) {
                            console.log(`Inserting extra ${extraId} for order item ${orderItemId}`);
                            await db.run("INSERT INTO order_item_extras (order_item_id, extra_id) VALUES (?, ?)", [orderItemId, extraId]);
                        }
...
electron_1.ipcMain.handle("update-order", async (_, order) => {
    try {
        const result = await withRetry(async () => {
            ...
            const orderItems = await db.all("SELECT oi.*, p.price FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?", [order.id]);
