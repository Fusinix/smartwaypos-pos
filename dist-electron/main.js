...
async function createWindow() {
    ...
    `);
        ...
        try {
            ...
            const hasFoodItemId = orderItemsColumns.some((col) => col.name === "food_item_id");
            const hasItemType = orderItemsColumns.some((col) => col.name === "item_type");
            const hasNotes = orderItemsColumns.some((col) => col.name === "notes");
            if (!hasFoodItemId) {
                await db.run("ALTER TABLE order_items ADD COLUMN food_item_id INTEGER");
            }
            if (!hasItemType) {
                await db.run("ALTER TABLE order_items ADD COLUMN item_type TEXT DEFAULT 'drink'");
            }
            if (!hasNotes) {
            ...
            const hasNullableProductId = orderItemsColumns.some((col) => col.name === "product_id" && col.notnull === 0);
...
electron_1.ipcMain.handle("update-settings", async (_, settings) => {
    ...
    try {
        ...
        if (!existingSettings) {
            ...
            await db.run("INSERT INTO settings (general, pos, theme) VALUES (?, ?, ?)", [
...
electron_1.ipcMain.handle("add-user", async (_, data) => {
    ...
    try {
        ...
        const result = await db.run("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", [user.username, hashedPassword, user.role]);
...
electron_1.ipcMain.handle("update-user", async (_, id, data) => {
    ...
    try {
        ...
        if (user.password) {
            const hashedPassword = await bcrypt.hash(user.password, 10);
            await db.run("UPDATE users SET username = ?, password = ?, role = ? WHERE id = ?", [user.username, hashedPassword, user.role, id]);
        }
        else {
...
electron_1.ipcMain.handle("request-password-reset", async (_, licenseKey) => {
    try {
    ...
    try {
        ...
        if ((importType === "all" || importType === "categories") &&
            ...
            for (const category of parsedData.categories) {
                await db.run("INSERT INTO categories (name, description, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)", [
                    category.name,
        ...
        if ((importType === "all" || importType === "products") &&
            ...
            for (const product of parsedData.products) {
                await db.run("INSERT INTO products (name, description, category, price, cost_price, stock, status, image, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
                    product.name,
                    product.description,
                    product.category,
                    product.price,
        ...
        if ((importType === "all" || importType === "orders") &&
            ...
            for (const order of parsedData.orders) {
                await db.run("INSERT INTO orders (sale_id, order_type, table_number, customer_name, payment_mode, tax, amount, amount_bt, status, admin_id, created_at, updated_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
                    order.sale_id,
                    order.order_type,
                    order.table_number,
                    order.customer_name,
            ...
            if (parsedData.order_items) {
                for (const item of parsedData.order_items) {
                    await db.run("INSERT INTO order_items (order_id, product_id, quantity, created_at) VALUES (?, ?, ?, ?)", [item.order_id, item.product_id, item.quantity, item.created_at]);
                }
            }
        }
...
electron_1.ipcMain.handle("clear-all-data", async (_, user) => {
    try {
...
electron_1.ipcMain.handle("clear-selective-data", async (_, options) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
electron_1.ipcMain.handle("update-category", async (_, { id, ...category }) => {
    try {
        // console.log('Updating category:', { id, category });
        if (!category.name) {
            throw new Error("Category name is required");
        }
        await db.run("UPDATE categories SET name = ?, description = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [category.name, category.description || null, category.status, id]);
...
electron_1.ipcMain.handle("delete-category", async (_, id, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
// Product handlers
electron_1.ipcMain.handle("get-products", async () => {
    try {
...
electron_1.ipcMain.handle("add-product", async (_, product, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
electron_1.ipcMain.handle("delete-product", async (_, id, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
electron_1.ipcMain.handle("get-low-stock-products", async () => {
    try {
        const products = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            return await db.all(`
...
electron_1.ipcMain.handle("get-out-of-stock-products", async () => {
    try {
        const products = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            return await db.all(`
...
electron_1.ipcMain.handle("update-product-stock", async (_, productId, newStock, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const oldStock = await db.get("SELECT stock, name FROM products WHERE id = ?", [productId]);
        if (!oldStock) {
...
electron_1.ipcMain.handle("get-food-categories", async () => {
    try {
        const categories = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            return await db.all(`
...
electron_1.ipcMain.handle("add-food-category", async (_, category, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const result = await db.run(`
    ...
    `, [
        ...
        const newCategory = await db.get("SELECT * FROM food_categories WHERE id = ?", [result.lastID]);
        const author = payload.author || {};
        await logAction({
            db,
            admin_id: author.id || null,
...
electron_1.ipcMain.handle("update-food-category", async (_, category, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        await db.run(`
    ...
    `, [
        ...
        const updatedCategory = await db.get("SELECT * FROM food_categories WHERE id = ?", [category.id]);
        const author = payload.author || {};
        await logAction({
            db,
            admin_id: author.id || null,
...
electron_1.ipcMain.handle("delete-food-category", async (_, id, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
// Food Item handlers
electron_1.ipcMain.handle("get-food-items", async () => {
    try {
...
electron_1.ipcMain.handle("add-food-item", async (_, item, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const result = await db.run(`
    ...
    `, [
        ...
        if (item.extra_ids &&
            ...
            console.log(`Inserting ${item.extra_ids.length} extras for food item ${result.lastID}`);
            for (const extraId of item.extra_ids) {
                console.log(`Inserting extra ${extraId} for food item ${result.lastID}`);
                await db.run("INSERT INTO food_item_extras (food_item_id, extra_id) VALUES (?, ?)", [result.lastID, extraId]);
            }
...
electron_1.ipcMain.handle("update-food-item", async (_, item, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        await db.run(`
      UPDATE food_items 
    ...
    `, [
        ...
        if (item.extra_ids &&
            Array.isArray(item.extra_ids) &&
            item.extra_ids.length > 0) {
            console.log(`Inserting ${item.extra_ids.length} extras for food item ${item.id}`);
            for (const extraId of item.extra_ids) {
                console.log(`Inserting extra ${extraId} for food item ${item.id}`);
                await db.run("INSERT INTO food_item_extras (food_item_id, extra_id) VALUES (?, ?)", [item.id, extraId]);
            }
            console.log(`Successfully inserted ${item.extra_ids.length} extras`);
        }
        else {
        ...
        const updatedItem = await db.get("SELECT * FROM food_items WHERE id = ?", [item.id]);
        const author = payload.author || {};
        await logAction({
            db,
            admin_id: author.id || null,
...
electron_1.ipcMain.handle("delete-food-item", async (_, id, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
// Food Extra handlers
electron_1.ipcMain.handle("get-food-extras", async () => {
    try {
...
electron_1.ipcMain.handle("add-food-extra", async (_, extra, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const result = await db.run(`
...
electron_1.ipcMain.handle("update-food-extra", async (_, extra, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        await db.run(`
      UPDATE food_extras 
    ...
    `, [extra.name, extra.price, extra.status, extra.id]);
        const updatedExtra = await db.get("SELECT * FROM food_extras WHERE id = ?", [extra.id]);
        const author = payload.author || {};
        await logAction({
            db,
            admin_id: author.id || null,
...
electron_1.ipcMain.handle("delete-food-extra", async (_, id, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
...
async function generateTakeOutTableNumber(db) {
    const now = new Date();
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, "0");
    // Get the last take-out table number for this month
    const lastTakeOutOrder = await db.get(`SELECT table_number FROM orders 
...
electron_1.ipcMain.handle("create-order", async (_event, order) => {
    try {
        ...
        if (Array.isArray(items) && items.length > 0) {
            ...
            for (const item of items) {
                if (item.itemType === "drink" && item.productId) {
                    ...
                    const foodItem = await db.get("SELECT price FROM food_items WHERE id = ?", [item.foodItemId]);
                    if (foodItem) {
        ...
        const lastOrder = await db.get(`SELECT order_number FROM orders 
        ...
        if (Array.isArray(items)) {
            for (const item of items) {
                if (item.itemType === "drink" && item.productId) {
                    ...
                    const oldStock = oldProduct?.stock || 0;
                    // Update product stock (reduce by ordered quantity)
                    await db.run("UPDATE products SET stock = stock - ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [item.quantity, item.productId]);
                    ...
                    if (item.extraIds &&
                        ...
                        for (const [extraId, qty] of extraCounts.entries()) {
                            await db.run("INSERT OR IGNORE INTO order_item_extras (order_item_id, extra_id, quantity) VALUES (?, ?, ?)", [orderItemId, extraId, qty]);
                        }
                    }
                }
            }
...
electron_1.ipcMain.handle("get-orders", async (_event, payload = {}) => {
    try {
...
electron_1.ipcMain.handle("get-order-by-id", async (_event, orderId, payload = {}) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            // Get the order
...
electron_1.ipcMain.handle("update-order", async (_, order) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            ...
            const drinkItems = await db.all("SELECT oi.*, p.price FROM order_items oi LEFT JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ? AND oi.item_type = 'drink'", [order.id]);
            const foodItems = await db.all("SELECT oi.*, fi.price as food_price FROM order_items oi LEFT JOIN food_items fi ON oi.food_item_id = fi.id WHERE oi.order_id = ? AND oi.item_type = 'food'", [order.id]);
...
electron_1.ipcMain.handle("bulk-update-orders", async (_, { ids, status, author }) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            const editorId = author?.id || null;
            for (const id of ids) {
                ...
                if (existingOrder.status !== "cancelled" &&
                    ...
                    const drinkItemsToRestore = await db.all("SELECT oi.product_id, oi.quantity FROM order_items oi WHERE oi.order_id = ? AND oi.item_type = 'drink'", [id]);
                    for (const item of drinkItemsToRestore) {
                        if (!item.product_id)
                            continue;
                        const currentProduct = await db.get("SELECT stock, name FROM products WHERE id = ?", [item.product_id]);
...
electron_1.ipcMain.handle("update-order-items", async (_, orderId, newItems, payload = {}) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            // Get current order items for stock adjustment
            const currentItems = await db.all("SELECT * FROM order_items WHERE order_id = ?", [orderId]);
            ...
            const allProductIds = new Set([
                ...currentItems
                    .filter((item) => item.item_type === "drink" && item.product_id)
                    .map((item) => item.product_id),
                ...newItems
                    .filter((item) => item.itemType === "drink" && item.productId)
            ...
            for (const item of newItems) {
                if (item.itemType === "drink" && item.productId) {
                    // Insert order item for drink
                    await db.run("INSERT INTO order_items (order_id, product_id, food_item_id, item_type, quantity, notes) VALUES (?, ?, ?, ?, ?, ?)", [orderId, item.productId, null, "drink", item.quantity, null]);
                }
                else if (item.itemType === "food" && item.foodItemId) {
                    // Insert order item for food
...
electron_1.ipcMain.handle("logout", async (_event, payload = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const author = payload.author || {};
        await logAction({
...
// Food stats handler
electron_1.ipcMain.handle("get-food-stats", async () => {
    try {
        const db = await (0, database_1.getDatabase)();
...
electron_1.ipcMain.handle("get-dashboard-stats", async (_event, filters = {}) => {
    ...
    `, [prevStartDateStr, prevEndDateStr]);
        ...
        const currentAverageOrder = closedOrdersCount > 0 ? currentRevenue / closedOrdersCount : 0;
        // Calculate previous period stats
        const prevRevenue = prevClosedOrders.reduce((sum, order) => sum + (order.amount || 0), 0);
        const prevOrdersCount = prevAllOrders[0]?.count || 0;
        const prevAverageOrder = prevClosedOrders.length > 0 ? prevRevenue / prevClosedOrders.length : 0;
        // Calculate changes
        const revenueChange = prevRevenue > 0 ? ((currentRevenue - prevRevenue) / prevRevenue) * 100
...
electron_1.ipcMain.handle("export-data", async (_event, { type, format, filters = {} }) => {
    try {
        const db = await (0, database_1.getDatabase)();
        let data;
        const cutoffHour = await getBusinessDayCutoffHour(db);
      ...
      `);
            ...
            const exportRows = [];
            const addSection = (title, columns, rows) => {
                exportRows.push({ __section: title });
                if (rows.length > 0) {
                    rows.forEach((row) => exportRows.push(row));
                }
                else {
                    exportRows.push(Object.fromEntries(columns.map((col) => [col, "No data"])));
                }
                exportRows.push({}); // blank row
            };
            // Revenue & Orders
            addSection("Revenue & Orders by Date", ["date", "revenue", "orders"], salesData);
            // Top Products
            addSection("Top Products", ["product", "category", "sold", "revenue"], topProducts);
            // Category Performance
            addSection("Category Performance", ["category", "revenue", "orders"], categoryPerformance);
            // Peak Hours
            addSection("Peak Hours", ["hour", "orders", "revenue"], peakHours);
            // Payment Methods
            addSection("Payment Methods", ["method", "count", "revenue"], paymentMethods);
            // Inventory Insights
            addSection("Inventory Insights", ["product", "category", "stock", "sold", "price", "stockValue"], inventoryInsights);
...
electron_1.ipcMain.handle("get-inventory-insights", async (_event, filters = {}) => {
    try {
        const db = await (0, database_1.getDatabase)();
        const inventoryInsights = await db.all(`
      SELECT 
...
electron_1.ipcMain.handle("add-table", async (_, table, payload = {}) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            const result = await db.run(`
      ...
      `, [table.name, table.capacity || null, table.status || "active"]);
        ...
        const author = payload.author || {};
        await logAction({
            db: await (0, database_1.getDatabase)(),
            admin_id: author.id || null,
            admin_name: author.name || null,
            admin_role: author.role || null,
            action: "add_table",
...
electron_1.ipcMain.handle("update-table", async (_, table, payload = {}) => {
    try {
        const result = await withRetry(async () => {
            const db = await (0, database_1.getDatabase)();
            await db.run(`
...
electron_1.ipcMain.handle("delete-table", async (_, tableId, payload = {}) => {
    try {
...
electron_1.ipcMain.handle("activate-license", async (_, licenseKey) => {
    try {
        const result = await licensing_1.licensingManager.validateLicense(licenseKey);
        ...
        await logAction({
            ...
            context: {
                ...
                hardwareId: licensing_1.licensingManager.getHardwareId(),
...
electron_1.ipcMain.handle("check-license", async () => {
    try {
        const result = await licensing_1.licensingManager.checkLicense();
        return result;
    }
    catch (error) {
...
electron_1.ipcMain.handle("get-hardware-id", async () => {
    try {
        return licensing_1.licensingManager.getHardwareId();
    }
    catch (error) {
...
electron_1.ipcMain.handle("get-license-info", async () => {
    try {
        return licensing_1.licensingManager.getLicenseInfo();
    }
    catch (error) {
...
electron_1.ipcMain.handle("get-last-online-check", async () => {
    try {
...
electron_1.ipcMain.handle("super-admin-login", async (_, credentials) => {
    try {
        ...
        if (result.success) {
            ...
            if (result.licenseInfo) {
                await licensing_1.licensingManager.validateLicense(credentials.licenseKey);
            }
...
electron_1.ipcMain.handle("force-license-validation", async (_, licenseKey) => {
    try {
        const result = await licensing_1.licensingManager.forceOnlineValidation(licenseKey);
        return result;
    }
    catch (error) {
...
electron_1.ipcMain.handle("trigger-cash-drawer", async (event) => {
    try {
        ...
        const printers = await event.sender.getPrintersAsync();
        const autoDetected = printers.find((p) => p.name.toUpperCase().includes("POS") ||
            p.name.includes("80") ||
...
electron_1.ipcMain.handle("print-receipt-silent", async (event, html, printerName) => {
